...
vec4 useLight(Light light, vec4 surfacePostion, vec4 normal_transformed, vec3 normal, mat4 inv_view, Material material)
{
    ...
    //attenuation
    float distanceToLight = length(light.position.xyz - surfacePostion.xyz);
...
void main(void)
{
    ...
    for (int i = 0; i < numLights; i++) {
        linearColor += useLight(lights[i], surfacePostion, transformedNormal, normal, inv_view, materials[0]);
    }
